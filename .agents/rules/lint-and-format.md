...
- **`missing-docs`**: every declaration with `package` or wider access under `Sources/<Package>Kit/**` and `Sources/<Package>CLI/**` needs a `///` doc comment. Exempt: all `init`s, `enum CodingKeys`, `var errorDescription`, `func ==`, and result-builder / interpolation hooks (`buildBlock`, `buildExpression`, `buildOptional`, `buildEither`, `buildArray`, `appendLiteral`, `appendInterpolation`). `*Generated.swift` is exempt.
