...
struct PlanningState: Encodable, Sendable {
    ...
    /// How to decide, which every question points at; see `JevStepPlanner.rules`. JSONEncoder does not keep key
    /// order, so this cannot be pinned before the data.
    let rules: String
    let goal: String
    let notes: [String]
    let platform: String
