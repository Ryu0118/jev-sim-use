...
struct SliderTests {
    /// A settings row: the name and the shown value above a slider whose label is its raw position.
    private let interval = Fixtures.entry(21, "Recording Interval", role: "StaticText", frame: ElementFrame(x: 32, y: 614, width: 64, height: 20))
    ...
    func caption() {
        ...
        #expect(snapshot.caption(ofSlider: slider) == SliderCaption(label: "Recording Interval", value: "25 m"))
        let element = PlanningState.Element(slider, slider: snapshot.caption(ofSlider: slider))
        #expect(element.label == "Recording Interval")
