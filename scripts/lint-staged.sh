...
EOF
    ...
    while IFS= read -r file; do
        [ -n "$file" ] || continue
        if ! OUTPUT=$("$SWIFTLINT" lint --config "$SOURCE_ROOT/.swiftlint.yml" --force-exclude --strict --quiet --no-cache "$SOURCE_ROOT/$file" 2>&1); then
...
EOF
    ...
    if [ "$LINT_FAILED" -ne 0 ]; then
        ...
        echo "SwiftLint reported issues. Fix them before committing." >&2
        exit 1
    fi

    if ! "$SOURCE_ROOT/scripts/ast-lint.sh"; then
        echo "my-swift-linter reported issues. Fix them before committing." >&2
        exit 1
    fi
fi

if ! "$DOCSYNC" check --config "$SOURCE_ROOT/docsync.yml"; then
    echo "docsync: docs are out of sync. Update docs or run 'mise run update-docsync-checksum'." >&2
    exit 1
fi
